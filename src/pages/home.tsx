...
export default function HomePoster() {
  ...
  return (
    <Layout>
      <HeaderHome />
      <Footer />
      <div className="flex flex-wrap w-screen p-4 justify-center h-auto lg:h-4/6 my-5 z-50">
        <div className="flex flex-wrap h-4/5 w-full justify-center">
          <div className={isClicked ? " flex flex-wrap w-10/12 h-fit lg:h-full lg:w-full justify-center mt-14 lg:m-20 lg:justify-between" : 'invisible'}>
          ...
          <div className={isClicked ? 'invisible' : " flex flex-wrap h-full w-full justify-center mt-14"}>

            <video className={isClicked1 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto lg:h-auto lg:w-1/4'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked1(true) }}>
              <source src="/video/CMPA2.mp4" type="video/mp4" />
            </video>
            <video className={isClicked2 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto lg:h-auto lg:w-1/4'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked2(true) }}>
              <source src="/video/detailing_core_2.mp4" type="video/mp4" />
            </video>
            <video className={isClicked3 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto lg:h-auto lg:w-1/4'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked3(true) }}>
              <source src="/video/detailing_core_3.mp4" type="video/mp4" />
            </video>
            <video className={isClicked4 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto lg:h-auto lg:w-1/4'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked4(true) }}>
              <source src="/video/detailing_pbf_2.mp4" type="video/mp4" />
            </video>
          </div>
        </div>

        <Link className={isClicked ? "bg-indigo-800 text-gray-200 w-2/3 lg:w-1/4 lg:mt-32 h-10 p-2 rounded-lg items-center flex justify-center" : 'hidden'} href="/qrcode">
          <button >
            Quiz
          </button>
        </Link>
