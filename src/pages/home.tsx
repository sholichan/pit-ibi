  ...
  return (
    <Layout>
      ...
      <div className="flex flex-wrap w-screen p-4 justify-center h-auto lg:h-4/6 my-5 z-50">
        <div className="flex flex-wrap h-4/5 w-full justify-center mt-10 lg:m-20 lg:justify-between">
          ...
          <video className={isClicked1 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked1(true) }}>
            <source src="/video/CMPA2.mp4" type="video/mp4" />
          </video>
          <video className={isClicked2 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked2(true) }}>
            <source src="/video/detailing_core_2.mp4" type="video/mp4" />
          </video>
          <video className={isClicked3 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked3(true) }}>
            <source src="/video/detailing_core_3.mp4" type="video/mp4" />
          </video>
          <video className={isClicked4 ? "hidden" : 'border-indigo-800 border-4 p1 w-5/6 h-auto'} autoPlay muted loop={true} width="400" height="200" onClick={() => { setClicked(true), setClicked4(true) }}>
            <source src="/video/detailing_pbf_2.mp4" type="video/mp4" />
          </video>
        </div>

        <Link className={isClicked ? "bg-indigo-800 w-2/3 lg:w-1/4 h-10 p-2 rounded-lg items-center flex justify-center" : 'hidden'} href="/qrcode">
          <button >
            Quiz
          </button>
        </Link>
